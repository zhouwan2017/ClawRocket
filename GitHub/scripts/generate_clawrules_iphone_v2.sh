#!/bin/bash
# 🦞 Clawrules iPhone 版生成脚本 v2.0（合并 v6 规则）
...
LOG_FILE="$LOG_DIR/generate_iphone_$(date +%Y%m%d-%H%M).log"
...
log "🦞 Clawrules iPhone 版 v2.0 生成器启动（合并 v6 规则）"
log "===================================================="

OUTPUT_FILE="$OUTPUT_DIR/🦞CR-i-$(date +%m%d-%H%M).conf"

# 生成配置头部
log "📝 生成 iPhone 版配置头部..."
cat > "$OUTPUT_FILE" << 'CONFIGHEAD'
# 🦞 Clawrules Shadowrocket 配置 iPhone 版 v2.0
# 生成时间: GENERATED_TIME
# 规则类型：白名单模式 (国内直连，其他代理)
# 版本：iPhone 标准版（合并 v6 完整规则）
#
# 优化说明:
# - DNS: 使用 DoH 防污染
# - 完整国内网站名单（2000+ 域名）
